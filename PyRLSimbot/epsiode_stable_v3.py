...
class RL_Robot(Robot):
    ...
    def __init__(self, **kwargs):
        ...
        for i in range(self.num_state_sensor**self.num_sensor):
                for label in (self.smell_labels):
                    state_index_list.append("{0}{1:05b}".format(label,i))
                    # state_index_list.append("{0}{1}".format(label,i))
        # print(state_index_list)
        print(f"state_index_list: {state_index_list}")
        # print(state_index_list)
    ...
    def episode_loop(self):
        ...
        print(f"self.pre_state_key: {self.pre_state_key}")
...
if __name__ == '__main__':
    ...
    , max_tick=100000, interval=1/240.0, enable_wasd_control = True,)
    app.run()
    ax1.plot(dataplot1)
